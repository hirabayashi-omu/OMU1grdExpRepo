...
st.markdown("""
    ...
    <div class="main-header">
        <h1>🧪 総合工学システム実験実習 M2<br><span style="-webkit-text-fill-color: #94a3b8; font-weight:400;">レポート作成システム</span></h1>
...
with st.sidebar:
    ...
    with st.container(border=True):
        ...
        **中断・再開用の個人バックアップ**
        ・**範囲**: 全テーマの全データ、更新履歴
        ・**保存**: ボタンでJSONをDL保存
        ・**復元**: ファイルを上げ「復元」ボタンを押す
        ⚠️ 復元すると現在の入力は上書き消去されます。
        """)
...
if st.session_state.exp_title == "実験① 熱の可視化":
    ...
    with st.expander("実験結果（アルカリ型燃料電池）"):
        st.markdown("#### 充電実験")
