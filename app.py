...
with st.sidebar:
    ...
    # 1. 作業状態の保存・復元
    with st.container(border=True):
        st.subheader("① 作業状態の保存・復元", help="""
        **【目的】**
        自分の作業を中断・再開するための自分専用バックアップです。
        
        **【保存範囲】**
        **全テーマ（①〜③）の全データ**、および更新履歴がすべて含まれます。
        
        **【復元の手順】**
        1. 保存したファイルを選択してアップロード。
        2. 「以前の入力状態を復元」ボタンを押す。
        3. ダイアログで「復元を実行」を押す。
        
        **⚠️ 注意**
        復元すると、現在入力中の内容はすべて消去され、ファイルの状態に上書きされます。
        """)
    ...
    # 2. 共有データの出力・復元
    with st.container(border=True):
        st.subheader("② 共有データの出力・復元", help="""
        **【目的】**
        班員と「実験データ」のみを効率よく共有・同期します。
        
        **【保存される範囲】**
        **現在選択中のテーマのみ**。「実験方法」と「実験結果」に特化しています。
        
        **【同期の手順】**
        1. 班員から共有用ファイルを受け取り、アップロード。
        2. 「共同実験者のデータを読み込む」ボタンを押す。
        3. ダイアログで提供者を確認し、「上書きを実行」を押す。
        
        **✨ 保持される範囲**
        あなたの氏名、出席番号、調査レポート、考察は上書きされず、そのまま残ります。
        """)
    ...
    with st.container(border=True):
        ...
        **【目的】**
        完成したレポートを提出するための正式なファイル（PDF）を作成します。
        
        **【注意点】**
        *   PDFを出力しても**作業状態は保存されません**。必ず「① 復元用ファイルの保存」も併せて行ってください。
        *   **PDFは必ず、自分自身の端末（自分の氏名・学籍番号が入力された状態）で作成したものを提出してください。**
        """)
