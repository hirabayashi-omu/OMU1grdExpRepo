...
with st.sidebar:
    ...
    with st.popover("❓ ヘルプ：操作メニューの使いかた", use_container_width=True):
        ...
        *   **保存の手順**: 「保存」ボタンを押し、作成されたファイルをダウンロードしてPCに保存します。
        *   **復元の手順**: 
            1. 保存したファイルを選択してアップロードします。
            2. 「以前の入力状態を復元」ボタンを押します。
            3. 確認ダイアログで「復元を実行」を押すと反映されます。
        ...
        *   **同期の手順**: 
            1. 班員が書き出した共有用ファイルを受け取り、選択してアップロードします。
            2. 「共同実験者のデータを読み込む」ボタンを押します。
            3. 確認ダイアログで提供者などを確認し、「上書きを実行」を押すと反映されます。
